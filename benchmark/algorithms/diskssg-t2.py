...
class Diskssg(BaseANN):
    ...
    def fit(self, dataset):
        ...
        if buildthreads == -1:
            buildthreads = diskssgpy.omp_get_max_threads()

        print("Set build-time number of threads:", buildthreads)
        diskssgpy.omp_set_num_threads(buildthreads)
        ...
        if ds.distance() == "euclidean":
            ...
            metric = diskssgpy.INNER_PRODUCT
        else:
        ...
        if not hasattr(self, 'index'):
            if ds.dtype == "float32":
                self.index = diskssgpy.DiskANNFloatIndex(metric)
            elif ds.dtype == "int8":
                self.index = diskssgpy.DiskANNInt8Index(metric)
            elif ds.dtype == "uint8":
                self.index = diskssgpy.DiskANNUInt8Index(metric)
            else:
        ...
        self.index.load_index(self.index_path, diskssgpy.omp_get_max_threads(), num_nodes_to_cache, self.cache_mechanism)
    ...
    def load_index(self, dataset):
        ...
        if self.C > 0:
            ...
            num_nodes_to_cache = int(ds.nb/1000) if ds.nb > 1000000 else int(ds.nb/100)
        if (self.index.load_index(index_path, diskssgpy.omp_get_max_threads(), num_nodes_to_cache, self.cache_mechanism) == 0):
